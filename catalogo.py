...
con vistas: Carpetas (árbol + grid de carpetas y grid de fotos), Cronología,
Búsqueda y visor modal. Añade skeleton loader y fade-in en las tarjetas
y mantiene la rejilla con altura fija (predecible).

USO:
    python catalogo.py [--root RUTA_RAIZ] [--outfile NOMBRE_HTML]
...
# --- Rich opcional para progreso bonito ---
RICH_AVAILABLE = True
try:
...
def scan_images(root_dir: str, html_dir: str):
    stats = {
        "folders_total": 0, "files_total": 0, "images_found": 0,
        "exif_ok": 0, "exif_fallback": 0, "errors": 0
    }
    per_year = Counter()
    ...
    if RICH_AVAILABLE:
        ...
        task_imgs  = progress.add_task("", total=0, title="Indexando imágenes")
        with progress:
            for fpath in files:
        ...
        for fpath in files:
            if not is_image(fpath):
                continue
            try:
                dt_exif = get_exif_datetime(fpath)
                dt = dt_exif or get_fs_datetime(fpath)
                if dt_exif: stats["exif_ok"] += 1
                else: stats["exif_fallback"] += 1
...
def build_html(photos_json: str, title: str = "Catálogo de Fotos"):
    """Plantilla HTML completa SIN f-strings (usa marcadores __TITLE__/__PHOTOS_JSON__)."""
    html = r"""<!doctype html>
<html lang="es"><head>
...
<style>
:root{
  --bg:#0e0f12; --bg-elev:#15171c; --panel:#1b1e25; --text:#e8e8ea; --muted:#a1a6b3;
  --accent:#7aa2ff; --border:#262a33; --chip:#232734; --shadow:0 10px 30px rgba(0,0,0,.35);
  --thumb-h:190px; /* alto fijo miniaturas */
}
...
.topbar{position:sticky;top:0;z-index:50;display:flex;align-items:center;justify-content:space-between;
  padding:12px 18px;background:linear-gradient(180deg,var(--bg-elev),rgba(14,15,18,.6));border-bottom:1px solid var(--border)}
.brand{font-weight:700;letter-spacing:.2px}
.view-btn{background:var(--chip);color:var(--text);border:1px solid var(--border);padding:8px 12px;border-radius:10px;cursor:pointer;margin-left:6px}
.view-btn[aria-pressed="true"]{outline:2px solid var(--accent)}
...
/* Thumb with skeleton */
.thumb-wrap{position:relative;width:100%;height:var(--thumb-h);overflow:hidden;background:#0a0b0d}
.thumb{width:100%;height:100%;object-fit:cover;opacity:0;transform:scale(1.02);transition:opacity .25s ease, transform .3s ease}
.thumb.is-loaded{opacity:1;transform:none}
.skel{position:absolute;inset:0;overflow:hidden}
.skel::before{content:"";position:absolute;inset:0;background:linear-gradient(110deg,#121317 8%,#1a1d24 18%,#121317 33%);background-size:200% 100%;animation:shimmer 1.1s linear infinite}
...
.folder-name{font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:70%}
...
.modal-body{background:var(--bg-elev);border:1px solid var(--border);border-radius:16px;max-width:min(92vw,1600px);max-height:86vh;display:flex;flex-direction:column;overflow:hidden}
...
// ---------- Loader + miniaturas ----------
function makeThumb(imgSrc, altText){
...
function createPhotoCard(photo, listForModal){
  ...
  card.className = 'card';
  card.appendChild(makeThumb(photo.src, photo.name));

  const title = document.createElement('div');
  title.className = 'title';
  title.innerHTML = `<span class="muted">${escapeHtml(photo.filename)}</span>
                     <span class="badge">${escapeHtml(photo.date_iso.replace('T',' ').substring(0,19))}</span>`;
...
// ---------- Carpetas (índices y árbol) ----------
function buildFolderIndex(){
  const idx = new Map(); // folder => {count, first}
...
function renderFolders(){
  ...
  layout.className = 'folders-layout';

  const sidebar = document.createElement('aside');
  sidebar.className = 'sidebar';
  sidebar.innerHTML = '<h3>Carpetas</h3>';
  const treeUl = document.createElement('ul');
  treeUl.className = 'tree';
  sidebar.appendChild(treeUl);

  const main = document.createElement('div');

  layout.appendChild(sidebar);
  layout.appendChild(main);
  elContent.appendChild(layout);
  ...
  function renderNode(ul, node, basePath){
    ...
    if(ul === treeUl && basePath === ''){
      ...
      btn.addEventListener('click', () => showFolder(''));
      li.appendChild(btn);
      ul.appendChild(li);
    }
    for(const name of names){
      ...
      btn.addEventListener('click', () => showFolder(full));
      li.appendChild(btn);
  ...
  function renderFolderTiles(){
    ...
    for(const folder of folders){
      const meta = folderIndex.get(folder);
      const first = meta.first;

      const card = document.createElement('div');
      card.className = 'card folder-tile';
      card.appendChild(makeThumb(first.src, folder || '(raíz)'));

      const title = document.createElement('div');
      title.className = 'title';
      title.innerHTML = `<span class="folder-name">${escapeHtml(folder || '(raíz)')}</span>
                         <span class="badge">${String(meta.count)} foto(s)</span>`;
      card.appendChild(title);
  ...
  function showFolder(path){
    ...
    if(!path){
      renderFolderTiles(); // grid de carpetas
      return;
    }
  ...
  // Por defecto, mosaico de carpetas
  showFolder('');
}
...
function renderChronology(){
  ...
  for(const Y of Object.keys(groups).sort()){
    ...
    yWrap.className = 'group';
    const yTitle = document.createElement('h4');
    yTitle.textContent = Y;
    yWrap.appendChild(yTitle);

    for(const M of Object.keys(groups[Y]).sort()){
      const mWrap = document.createElement('div');
      mWrap.style.marginBottom = '10px';
      const mTitle = document.createElement('div');
      mTitle.className = 'badge';
      mTitle.textContent = Y + '-' + M;
      mWrap.appendChild(mTitle);

      for(const D of Object.keys(groups[Y][M]).sort()){
        const dTitle = document.createElement('div');
        dTitle.className = 'muted';
        dTitle.style.margin = '8px 0 6px';
        dTitle.textContent = Y + '-' + M + '-' + D;
        mWrap.appendChild(dTitle);

        const sub = document.createElement('div');
        sub.className = 'subgrid';

        const list = groups[Y][M][D].slice().sort(byDate);
        const listForModal = list.slice();
...
function renderSearch(){
  ...
  btnClear.onclick = () => { q.value=''; dateFrom.value=''; dateTo.value=''; run(); };
...
function openModal(list, index){
  ...
  modalDate.textContent = p.date_iso.replace('T',' ').substring(0,19);
...
function nav(delta){
  ...
  if (i < 0) i = state.currentList.length - 1;
  if (i >= state.currentList.length) i = 0;
  openModal(state.currentList, i);
}
modalClose.addEventListener('click', closeModal);
modalPrev.addEventListener('click', () => nav(-1));
modalNext.addEventListener('click', () => nav(1));
...
switchView('folders');
    ...
    return html.replace("__TITLE__", title).replace("__PHOTOS_JSON__", photos_json)

def main():
    ap = argparse.ArgumentParser(description="Genera un único index.html con catálogo fotográfico.")
    ...
    # Escaneo con rutas relativas al HTML de salida
    ...
    try:
        webbrowser.open("file://" + out_html, new=2)
